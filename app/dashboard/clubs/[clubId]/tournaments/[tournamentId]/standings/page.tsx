...
function FormBadge({ result }: { result: string }) {
  const c: Record<string, string> = { W: "bg-win text-white", D: "bg-draw text-white", L: "bg-loss text-white" };
  return <span className={`inline-flex items-center justify-center w-5 h-5 rounded text-[9px] font-bold ${c[result] ?? "bg-muted text-muted-foreground"}`}>{result}</span>;
}

export default function TournamentStandingsPage({ params }: { params: Promise<{ clubId: string; tournamentId: string }> }) {
            ...
            <tbody className="divide-y divide-border/50">
              {standings.map((row) => (
                <tr key={row.teamId} className="hover:bg-accent/20 transition-colors">
                  ...
                  {[row.played, row.won, row.drawn, row.lost].map((v, i) => <td key={i} className="px-3 py-3 text-center text-muted-foreground">{v}</td>)}
                  <td className="px-3 py-3 text-center">
                    <span className={row.goalDifference > 0 ? "text-win" : row.goalDifference < 0 ? "text-loss" : "text-muted-foreground"}>
